...
interface WishlistContextType {
   ...
   */
  add: (trickId: string, optimisticData?: Partial<WishlistItem>) => Promise<void>;
...
export function WishlistProvider({ children }: { children: React.ReactNode }) {
  ...
  const add = useCallback(async (trickId: string, optimisticData?: Partial<WishlistItem>) => {
        ...
        if (inserted) {
          try {
            ...
            if (!trickError && trickData) {
              setItems((prev) =>
                prev.map((t) => (t.id === trickId ? { ...t, ...trickData } : t))
              );
            }
          } catch (e) {
  ...
  const remove = useCallback(async (trickId: string) => {
    ...
    try {
      const { removeFromWishlist } = await import("@/lib/client/wishlist-client");
  ...
  return (
    <WishlistContext.Provider value={{ items, loading, adding, removing, add, remove, refresh, lastAddedId, clearLastAdded }}>
      {children}
