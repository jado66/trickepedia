...
export function DeleteBugButton({ id }: DeleteBugButtonProps) {
  ...
  const handleDelete = async () => {
    ...
    if (result.success) {
      toast({
        ...
        description: "The bug report has been successfully removed.",
      });
      setOpen(false);
    } else {
      toast({
        title: "Error",
        description: result.error || "Failed to delete bug report",
        variant: "destructive",
      });
    }
    setIsDeleting(false);
    ...
    <AlertDialog open={open} onOpenChange={setOpen}>
      <AlertDialogTrigger asChild>
        ...
        >
          ...
          <span className="sr-only">Delete bug report</span>
        </Button>
      </AlertDialogTrigger>
      <AlertDialogContent>
        <AlertDialogHeader>
